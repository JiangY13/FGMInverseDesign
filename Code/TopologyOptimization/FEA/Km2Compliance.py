...
class Sk2Complicance(torch.autograd.Function):
    ...
    def forward(ctx, sK, indices, f, f_th, Ksize):
        ...
        U = torch.as_tensor(B_cpu, dtype=torch.float32, device=sK.device)
        ctx.U, ctx.row_indices, ctx.col_indices = U, row_indices, col_indices
        c = torch.dot(U, f_th)
        return c

    @staticmethod
    def backward(ctx, grad_c):
        U, row_indices, col_indices = ctx.U, ctx.row_indices, ctx.col_indices
        # _manual_grad_c, = ctx.saved_tensors
        grad_C = U[row_indices]
        grad_C.mul_(U[col_indices])
        grad_C.mul_(-grad_c)
